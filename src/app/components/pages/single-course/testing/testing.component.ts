...
export class TestingComponent {
  ...
  constructor(private _lightbox: Lightbox,private titleService: Title, private metaService: Meta, private el: ElementRef){
    const captions = [""];
    const conts = [""];
    for (let i = 1; i <= 4; i++) {
...
// for curricullum end
    ...
    // set the title
    this.titleService.setTitle('Learn Best Digital Marketing Course in Hyderabad - Teks academy');

    // set meta description
    this.metaService.addTag({name: 'description', content: 'SEO is a part of Digital marketing and Teks academy provides in-depth knowledge of Search engine optimization courses in Hyderabad to become an expert in SEO.'});
